...
module.exports = [
...

  {
    ...
    struct: node.op("+", [
      1,
      node.op("^", [node.mem(["point", "component_1"]), node.pOP("!", [2])]),
    ]),
  },

  {
    math: "1 + point1.  func()",
    parserOptions: { singleCharName: false },
    struct: node.op("+", [1, node.mem(["point1", node.F("func", [])])]),
  },

  {
    ...
    struct: node.op("+", [
      1,
      node.mem(["point1", node.F("func", [node.op("+", [1.2, "x"]), "s"])]),
    ]),
  },
