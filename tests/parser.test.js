  ...
  toHaveStructure(node, struct) {
    function failed(msg) {
      ...
      function simplify(o) {
        ...
        for (const p in o) {
          if (ignore.indexOf(p) === -1 && Object.prototype.hasOwnProperty.call(o, p)) {
            r[p] = simplify(o[p]);
          }
        }
        return r;
    ...
    function _check(n, s, nPath, sPath) {
      ...
      if (!(s instanceof Object)) {
        return failed(
          `"struct" is type of ${typeof s}, toHaveStructure checks the match between parser.Node and object.`,
          node
        );
      }
      ...
      if (!n.check(s)) {
        return failed(
          `properties of ${nPath} in node, don't match these of ${sPath} of struct`,
          node
        );
      }
    ...
    return (
      _check(node, struct) || {
        message: () => "Parse-tree matches the structure object!",
        pass: true,
      }
    );
  },
...
function getTitle(__test) {
  ...
  return (__test.error ? "should throw: " : "should parse: ") + JSON.stringify(__test.math);
}
  ...
  describe(title, () => {
    ...
    if (on instanceof Array) {
      on.forEach((__test) => {
        ...
        let fn = () => {
          ...
          if (__test.error) {
            expect(() => parser.parse(__test.math, __test.parserOptions)).toThrow(
              __test.errorType === "syntax"
                ? parser.SyntaxError
                : __test.errorType === "options"
                ? parser.OptionsError
                : __test.errorType
            );
          }
        ...
        if (__test.only) test.only(title, fn);
        else if (__test.skip) test.skip(title, fn); // finally test as usual
        else test(title, fn); // finally test as usual
      });
    }

    // we have another sub-group, `jest.descripe`
    else if (typeof on === "object") for (let p in on) doTest(on[p], p);
    // unexpected thing happened
    else throw new Error("can't do tests on " + typeof on);
  });
}
